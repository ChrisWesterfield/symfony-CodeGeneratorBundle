...
class EntityRepositoryGenerator extends GeneratorAbstract implements GeneratorInterface
{
    ...
    {
        /** @var CG\Entity\Entity $workingAnnotation */
        $workingAnnotation = $this->getAnnotation();
        if(empty($workingAnnotation->getRepository()))
        {
            $workingAnnotation->setRepository($this->getDocumentAnnotation()->getClassShort().self::REPOSITORY_POSTFIX);
        }
        if(empty($workingAnnotation->getTargetNameSpace()))
        {
            $workingAnnotation->setTargetNameSpace($this->getDocumentAnnotation()->getBundleRootNamespace().'\\'.self::REPOSITORY_DIRECTORY);
        }
        ...
        $output = $this->getRenderer()->renderTemplate("MjrOneCodeGeneratorBundle:Entity:Repository.php.twig",$templateVariables);
        ...
        {
            throw new \RuntimeException('Entity Annotation ORM\Entity or '.Entity::class.' was not defined in Class '.$this->getDocumentAnnotation()->getFqdnName().'! It could not be added. You neet to insert it on your own!');
        }
        ...
        foreach($oldFileArray as $row)
        {
            ...
            {
                $annotationString = ' * @ORM\Entity(repositoryClass="'.$fqdn.'", readOnly='.($searchedAnnotation->readOnly?'true':'false').');';
                $newFile[] = $annotationString;
            }
            else
                if(strpos($row,Entity::class)!==false)
            {
                $annotationString = ' * @'.Entity::class.'(repositoryClass="'.$fqdn.'", readOnly='.($searchedAnnotation->readOnly?'true':'false').');';
                $newFile[] = $annotationString;
            }
            else
            {
