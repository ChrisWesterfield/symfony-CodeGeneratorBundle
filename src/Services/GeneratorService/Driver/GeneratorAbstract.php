...
abstract class GeneratorAbstract implements GeneratorInterface
{
    ...
    protected function getBasics(array $templateVariables, string $className): array
    {
        ...
        $data = [
            'namespace'     => $doc->getBundleRootNamespace() . '\\' . Annotation::TRAIT_NAMESPACE . $doc->getClassNamespacePath(),
    ...
    {
        return method_exists($this->kernel,'getRealRootDirectory')?$this->kernel->getRealRootDirectory():$this->kernel->getRootDir().'/../';
    }
    ...
    {
        return [$this->getRootBundlePath($file),$traitPrefix.$this->getDocumentAnnotation()->getClassShort().self::FILE_EXTENSION];
    }
