...
class ServicePropertiesGenerator extends SubGeneratorAbstract implements SubDriverInterface
{
    ...
    public function process():void
    {
        ...
        $event = (new ServicePropertiesGeneratorEvent())->setConfig($this->config)->setTemplateVariables($templateProperties)->setSubject($this);
        ...
        foreach($this->getDocumentAnnotation()->getProperties() as $property)
        {
            if($property->getAnnotations()->count() > 0)
            {
                ...
                $event = (new ServicePropertiesGeneratorEvent())->setConfig($this->config)->setTemplateVariables($templateProperties)->setSubject($this)->setAnnotation($annotation);
