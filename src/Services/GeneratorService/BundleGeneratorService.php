...
namespace MjrOne\CodeGeneratorBundle\Services\GeneratorService;
...
class BundleGeneratorService extends AbstractService implements CodeGeneratorInterface
{
    ...
    public function process()
    {
        ...
        {
            throw new BundleDoesNotExistException('The Bundle with the path '.$this->getBundle().' does not exist!');
        }
        $finder = new Finder();
        ...
        foreach($finder as $file)
        {
            ...
            if($returnCode!==0)
            {
                throw new \RuntimeException('Command not finished Executing 0. Parameters: '.json_encode($argument).' - file '.$file->getRealPath().'/'.$file->getBasename());
            }
        }
    }
}
