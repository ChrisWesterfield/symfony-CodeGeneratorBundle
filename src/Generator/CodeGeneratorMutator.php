...
class CodeGeneratorMutator extends CodeGeneratorAbstract implements CodeGeneratorInterface
{
    ...
    public function process():void
    {
        ...
        /** @var CG\Mutator $classAnnotation */
                ...
                if (!empty($propertyItem))
                {
                    ...
                    foreach ($propertyItem->getAnnotations() as $property)
                    {
                        if ($property instanceof CG\PropertyInterface and $property instanceof CG\Mutator\Property)
                        {
                            ...
                            $this->getED()->dispatch(
                                $this->getED()->getEventName(self::class, 'process.setPropertyAnnotation'), $event
                            );
                    ...
                    if ($propertyAnnotation === null)
                    {
                        ...
                        $this->getED()->dispatch(
                            $this->getED()->getEventName(self::class, 'process.setEmptyPropertyAnnotation'), $event
                        );
                        $this->getED()->dispatch(
                            $this->getED()->getEventName(self::class, 'process.setPropertyAnnotation'), $event
                        );
                    ...
                    foreach ($propertyKeys as $field)
                    {
                        if ($propertyAnnotation->{$field} === null && !in_array($field, self::IGNORED, true))
                        {
                            ...
                            $this->getED()->dispatch(
                                $this->getED()->getEventName(self::class, 'process.getDefaultClassAnnotationValue'),
                                $event
                            );
                    ...
                    if (!$propertyAnnotation->hasType())
                    {
                        ...
                        else
                        {
                            ...
                            {
                                $propertyAnnotation->setArrayType(
                                    $propertyType->getCollectionValueType()->getBuiltinType()
                                );
                            }
                        }
                    }
                    ...
                    $this->getED()->dispatch(
                        $this->getED()->getEventName(self::class, 'process.preAddPropertyItem'), $event
                    );
                    $propertyArray = [
                    ...
                    $this->getED()->dispatch(
                        $this->getED()->getEventName(self::class, 'process.prepareAddArray'), $event
                    );
