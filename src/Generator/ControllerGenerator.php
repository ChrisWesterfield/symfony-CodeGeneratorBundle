...
class ControllerGenerator extends GeneratorAbstract
{
    ...
    ): void
    {
        ...
        $sourceDir = '';
        if ($this->hasBundleSrcDirectoryStructure($dir))
        {
            $sourceDir = '/' . self::DIRECTORY_SRC;
        }

        $controllerFile =
            $dir . $sourceDir . '/' . self::DIRECTORY_CONTROLLER . '/' . $controller . self::FILE_EXTENSION_CONTROLLER
        ...
        foreach ($actions as $i => $action)
        {
            ...
            {
                @trigger_error(
                    'The use of the "default" keyword is deprecated. Use the real template name instead.',
                    E_USER_DEPRECATED
                );
                $template = $bundle->getName() . ':' . $controller . ':' .
                            strtolower(
                                preg_replace(
                                    array('/([A-Z]+)([A-Z][a-z])/', '/([a-z\d])([A-Z])/'), array('\\1_\\2', '\\1_\\2'),
                                    strtr(substr($action['name'], 0, -6), '_', '.')
                                )
                            )
                            . '.html.' . $templateFormat;
            }
            ...
            {
                $this->renderFile(
                    self::BUNDLE_TWIG_NAMESPACE . 'template.html.twig.twig',
                    $dir . $sourceDir . '/Resources/views/' . $this->parseTemplatePath($template),
                    $params
                );
            }
            else
            {
                $this->renderFile(
                    self::BUNDLE_TWIG_NAMESPACE . 'template.html.php.twig',
                    $dir . $sourceDir . '/Resources/views/' . $this->parseTemplatePath($template),
                    $params
                );
            }
        ...
        $this->renderFile(
            self::BUNDLE_TWIG_NAMESPACE . '/controller.php.twig',
            $controllerFile,
            $parameters
        );
        $this->renderFile(
            self::BUNDLE_TWIG_NAMESPACE . '/controllerTest.php.twig',
            $dir . '/' . self::DIRECTORY_TESTS . '/' . self::DIRECTORY_CONTROLLER . '/' . $controller
            . self::FILE_EXTENSION_CONTROLLER . self::FILE_TEST . self::FILE_EXTENSION_PHP,
            $parameters
        );
    }/** @noinspection MoreThanThreeArgumentsInspection */
     ...
     */
    public function generateRouting(BundleInterface $bundle, $controller, array $action, $format): bool
    {
...
EOT;
        ...
        if ($flink)
        {
            ...
            else
            {
                throw new RuntimeException(
                    sprintf('We cannot write into file "%s", has that file the correct access level?', $file)
                );
            }
        }
        else
        {
            throw new RuntimeException(
                sprintf('Problems with generating file "%s", did you gave write access to that directory?', $file)
            );
        }
     ...
     * @return string
     */
    protected function parseTemplatePath($template): string
    {
    ...
    protected function parseLogicalTemplateName($logicalName, $part = '')
    {
        ...
        {
            throw new RuntimeException(
                sprintf('The given template name ("%s") is not correct (it must contain two colons).', $logicalName)
            );
        }
